...
- if branch protection requires PR checks on Release Please PRs, add a `RELEASE_PLEASE_TOKEN` repository secret and swap the workflow token from `github.token` to that secret
- treat the release workflow as the normal GitHub Pages deploy path for production docs
- use `.github/workflows/deploy.yml` only when you need to redeploy a specific branch, tag, or commit manually
- confirm the published GitHub release includes the expected autogenerated notes and attached `dist/` tarball
