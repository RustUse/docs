...
- approve the protected `release` environment when the candidate is ready to publish
- confirm the generated GitHub release includes the expected autogenerated notes and attached `dist/` tarball

The human approval step depends on the repository `release` environment having required reviewers configured in GitHub settings.
