...
<MascotTip
  title="Focused geometry set"
  description="use-geometry is the direct 2D crate and current set anchor for RustUse geometry. Its public status is still scaffolded, so RustUse-hosted generated rustdocs remain the canonical API reference while crates.io and docs.rs targets stay staged."
/>
...
`use-geometry` is the supported RustUse crate for small 2D Euclidean geometry primitives, validated construction at API boundaries, and direct measurement helpers. Today it also anchors the `use-geometry` set.
...
- [Open generated workspace Rustdocs for use-geometry](/api/workspaces/use-geometry/)
