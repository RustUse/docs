...
## Future: add a set, not individual crates

Once [`rustuse`](/rustuse/) is published on crates.io, adding the `use-math` set will be a single dependency declaration:
...
You add the set you want by name. The umbrella crate handles the rest — there is no need to list focused crates like `use-geometry` or `use-combinatorics` separately.
...
- [use-geometry crate page](/use-math/use-geometry/)
