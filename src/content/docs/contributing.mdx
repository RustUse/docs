...
- RustUse set and crate links inside `public/llms.txt` generated regions are generated from sibling `use-*` workspaces.
...
- Live crates.io package records control publication status and external package links.
- `scripts/sync-crate-surface.mjs` generates crate catalog data, overview pages, and `docs/rustdoc-sources.json`.
- `scripts/generate-llms-txt.mjs` updates the generated RustUse inventory regions in `public/llms.txt`.
...
- The generated catalog keeps a crate non-published until the live crates.io package record matches the local package repository URL.
- A fully published set is added to generated RustDocs inputs automatically by `npm run sync:crate-surface`.
...
`public/llms.txt` keeps its title, primary links, and Rustdoc URL patterns as
hand-authored content. The RustUse set and crate lists are generated between
the `BEGIN GENERATED RUSTUSE ...` and `END GENERATED RUSTUSE ...` markers.

Run `npm run generate:llms` after adding, removing, or renaming a sibling
RustUse set or crate workspace. CI runs `npm run check:llms` and fails when the
generated regions are stale. Keep manual edits outside the generated markers.
...
- Run `npm run sync:crate-surface` and `npm run generate:llms`, then `npm run build`, and verify the generated catalog, `llms.txt`, and stable RustDocs route resolve without a 404.
