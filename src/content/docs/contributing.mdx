...
workspace Rustdoc URL patterns, and generated RustUse sets only.
`public/llms-full.txt` carries the expanded root context with generated sets and
crates. Per-set files are generated at both `public/{set}/llms*.txt` and
`public/sets/{set}/llms*.txt`, with aliases serving the same generated content
...
- Run `npm run sync:crate-surface` and `npm run generate:llms`, then `npm run build`, and verify the generated catalog, root and full LLM files, and stable RustDocs route resolve without a 404.
